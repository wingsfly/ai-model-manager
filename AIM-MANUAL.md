...
aim root add /Volumes/Backup/AI --label backup --removable   # 外置/可移除盘(供 offload 用)
```

添加新根会自动创建 `.aim/` 和 `store/` 子目录。`--removable` 标记该根为外置/可移除盘,离线归档(`aim offload`)的模型放在这类根上。
...
> `migrate` 适合「多块盘各自跑引擎」的常驻布局(把模型连同引擎链接整体搬到目标盘)。若只是想把不常用的模型**归档到外置冷盘、腾主盘、需要时再搬回**,用下面的 `aim offload`。

---

### aim offload — 离线归档到外置盘 / 搬回

把不常用的模型归档到**可移除根**(外置盘),释放主盘空间;需要时一键搬回,全程 aim 感知、可逆、不损索引。

```bash
# 一次性:把外置盘加为可移除根
aim root add /Volumes/Backup/AI --label backup --removable

# 归档:数据搬到外置盘,主盘释放,模型标记为 offline
aim offload hf-mistralai-voxtral-small-24b-2507 --to backup

# 搬回:数据回主盘,重建引擎链接,无缝回归 aim 管理
aim offload hf-mistralai-voxtral-small-24b-2507 --restore

# 查看所有已归档模型及外置盘挂载状态
aim offload --list

# 归档时保留原 HF/MS 缓存字节(默认清除以腾空间)
aim offload <id> --to backup --keep-cache
```

**机制：**
- **native_cas 模型**(HF/MS 缓存)会**直接 ingest 到外置盘**(边纳管边落到外置根),主盘不产生瞬时第二份副本——即使主盘快满也安全。
- **已管理的 store 模型**则把 store 目录移动到外置根,并移除主盘上的引擎链接。
- 记录原始下载来源(`hf:...` / `ms:...`),即使外置盘丢失也知道去哪重新下载。

**离线感知(核心):**
- `list` / `info` 对离线模型显示 `⚠ offline (on <root>)`;外置盘未挂载时显示 `⚠ offline (<root> 未挂载)`。
- `verify` / `orphans` **自动跳过**离线模型——外置盘拔掉也**不会误报 missing、不会损坏索引**。
- 搬回时若外置盘未挂载,会提示挂盘或用 `aim download <source>` 重新下载。

**安全:** 全程先拷后删 + 注册表自动备份,中断不丢数据。
